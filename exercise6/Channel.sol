...
contract Channel {
    ...
    address payable public owner1;  // The one that opened the channel (and deposited the initial funds).
    address payable public owner2;

    // After 'appeal_period_len' blocks the funds can be withdrawn (and no appeal can be made).
    uint public appeal_period_len;

    // This mapping holds the balances of each of the owners, that can be withdrawn (after the appeal period ends).
    mapping (address => uint256) private userBalances; 

    // The serial number of the last split of the balance between the two owners.
    // One of the owners can appeal with a different balances split with a larger serial number.
    int8 public last_serial_num;

    bool public channel_open;     // Is the channel open or closed.
    uint public block_number_at_closure;  // The block number at closure time.

    modifier onlyOwners{
        require(msg.sender == owner1 || msg.sender == owner2, "Only an owner can call this function.");
        _;
    }

    modifier openChannel{
        require(channel_open, "This function must be called when the channel is open.");
        _;
    }
    ...
    constructor(address payable _other_owner, uint _appeal_period_len) payable public {
        // TODO are the following assertions necessary?
        // require(msg.value > 0, 
    ...
    function default_split() onlyOwners openChannel external {
        // Closes the channel according to a default_split, gives the money to party 1. 
    ...
    function one_sided_close(uint256 balance, int8 serial_num , uint8 v, bytes32 r, bytes32 s) onlyOwners openChannel external {
    ...
    function appeal_closure(uint256 balance, int8 serial_num , uint8 v, bytes32 r, bytes32 s) onlyOwners closedChannel appealPeriod external {
    ...
    function withdraw_funds(address payable dest_address) onlyOwners closedChannel appealPeriodEnded external {
    ...
    function verify_sig(uint256 balance, int8 serial_num, uint8 v, bytes32 r, bytes32 s, address signerPubKey) pure public returns (bool) {
        bytes32 hashMessage = keccak256(abi.encodePacked(address(this), balance, serial_num));
    ...
    // public variables in Solidity have auto-generated getters, we can remove our own getters.
