...
def test_transaction_to_myself(bank: Bank, alice: Wallet, alice_coin: Transaction) -> None:
    tx = alice.create_transaction(alice.get_address())
    assert tx is not None
    assert bank.add_transaction_to_mempool(tx)
    bank.end_day()
    alice.update(bank)
...
def test_chain_transactions(bank: Bank, alice: Wallet, bob: Wallet, charlie: Wallet, alice_coin: Transaction) -> None:
    ...
    assert alice.get_balance() == 0
    assert bob.get_balance() == 1
    tx1 = bob.create_transaction(charlie.get_address())
    assert tx is not None
    assert bank.add_transaction_to_mempool(tx1)
    assert bank.get_mempool() == [tx1]
    bank.end_day()
    bob.update(bank)
    charlie.update(bank)
    assert bank.get_utxo() == [tx1]
    assert charlie.get_balance() == 1
    assert bob.get_balance() == 0
    assert bank.get_mempool() == []
